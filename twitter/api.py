...
class Api(object):
    ...
    def GetSearch(self,
        ...
        if len(data['statuses']) < count:
          data['search_metadata']['isMoreAvailable'] = False
          data['search_metadata']['min_id'] = data['statuses'][-1:][0]['id']
        elif len(data['statuses']) == count:
          if str(data['statuses'][-1:][0]['id']) != "None":
            data['search_metadata']['isMoreAvailable'] = True
            data['search_metadata']['min_id'] = data['statuses'][-1:][0]['id']

        if return_json:
            return data
        else:
            return [Status.NewFromJsonDict(x) for x in data.get('c', '')]
        # return data
